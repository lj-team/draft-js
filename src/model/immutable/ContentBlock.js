...
class ContentBlock extends ContentBlockRecord {
  ...
  setKey(newKey: string): ContentBlock {
    const childBlockMap = this.getChildBlockMap();
    const newChildBlockMap = childBlockMap.map(b => b.merge({
      parentKey: newKey,
    }));
    return this.merge({
      key: newKey,
