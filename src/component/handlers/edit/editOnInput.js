 ...
 */
function editOnInput(editor: DraftEditor, e: SyntheticEvent): void {
  if (editor.props.handleInput) {
    const output = editor.props.handleInput(editor, e);
    if (output === 'handled') {
      return;
    }
  }
  ...
  const mutableEntities = entitySet.filter(entityKey => {
    const contentEntityData = content.getEntity(entityKey);
    if (
      contentEntityData &&
      contentEntityData.getMutability() === 'MUTABLE'
    ) {
      return true;
    }
    return false;
  });
  const allEntitiesAreMutable = mutableEntities.equals(entitySet);

  // Immutable or segmented entities cannot properly be handled by the
  // default browser undo, so we have to use a different change type to
  // force using our internal undo method instead of falling through to the
  // native browser undo.
  const changeType = allEntitiesAreMutable ? 'spellcheck-change' : 'apply-entity';
